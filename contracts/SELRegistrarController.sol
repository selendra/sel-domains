...
contract SELRegistrarController {
    ...
    bytes32 public constant SEL_NODE = 0x6e5e8a6db7e0f8f8c9c4e5d6b7a8e9f0a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6;
    ...
    constructor(ISNSRegistry _sns, address _baseRegistrar, IPriceOracle _priceOracle) {
        sns = _sns;
    ...
    function available(string memory name) public view returns (bool) {
        ...
        bytes32 node = keccak256(abi.encodePacked(SEL_NODE, label));
        return valid(name) && sns.owner(node) == address(0);
    }
    ...
    function register(
        ...
        bytes32 commitment = makeCommitment(name, owner, duration, secret, resolver, data, reverseRecord);
        
        // Verify commitment
        uint256 commitTime = commitments[commitment];
        require(commitTime > 0, "No commitment found");
        require(block.timestamp >= commitTime + MIN_COMMITMENT_AGE, "Commitment too new");
        ...
        // Set the owner in registry
        bytes32 node = sns.setSubnodeOwner(SEL_NODE, label, owner);
        
        // Set resolver if specified
    ...
    function renew(string calldata name, uint256 duration) external payable {
        ...
        require(sns.owner(node) != address(0), "Name not registered");
        
        (uint256 base, ) = rentPrice(name, duration);
        require(msg.value >= base, "Insufficient payment");
        
        // Note: In production, would interact with BaseRegistrar to extend expiry
        // For now, emit event with new expiry
        uint256 newExpires = block.timestamp + duration;
        
        emit NameRenewed(name, label, base, newExpires);
        
        // Refund excess
        if (msg.value > base) {
            payable(msg.sender).transfer(msg.value - base);
        }
    }
    ...
    function _setRecords(address resolver, bytes32 node, bytes[] calldata data) internal {
...
interface IPriceOracle {
    function price(string memory name, uint256 duration) external view returns (uint256 base, uint256 premium);
}
...
contract SimplePriceOracle is IPriceOracle {
     ...
     */
    function price(string memory name, uint256 duration) external pure override returns (uint256 base, uint256 premium) {
